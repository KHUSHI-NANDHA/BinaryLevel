...
def init_db():
    ...
    # Add missing columns to existing local_guides table
    try:
        cursor.execute('ALTER TABLE local_guides ADD COLUMN home_country TEXT')
    except:
        pass  # Column already exists
    
    try:
        cursor.execute('ALTER TABLE local_guides ADD COLUMN field_of_study TEXT')
    except:
        pass  # Column already exists
    
    try:
        cursor.execute('ALTER TABLE local_guides ADD COLUMN languages TEXT')
    except:
        pass  # Column already exists
    
    
    try:
        cursor.execute('ALTER TABLE local_guides ADD COLUMN cultural_background TEXT')
    except:
        pass  # Column already exists
    ...
    if user_count == 0:
        ...
        demo_profiles = [
            (local_user_ids[0], 'Prague', 'Czech Republic', 'Charles University', None, 'Experienced guide helping students with housing, transport, and cultural adaptation in Prague.', 2.0, 1, 'approved', 'India', 'Computer Science', 'English, Hindi, Czech', 'From Mumbai, understands the challenges of adapting to European culture'),
            (local_user_ids[1], 'Budapest', 'Hungary', None, 'Google', 'Local expert specializing in student life, affordable shopping, and public transport tips.', 2.0, 1, 'approved', 'India', 'Engineering', 'English, Hindi, Hungarian', 'Originally from Delhi, helps students with cultural integration'),
            (local_user_ids[2], 'Warsaw', 'Poland', 'Warsaw University', None, 'Helping students find affordable housing and navigate Polish bureaucracy with ease.', 2.0, 1, 'approved', 'India', 'Business', 'English, Hindi, Polish', 'From Bangalore, specializes in helping students with academic and professional development')
        ]
        
        for profile_data in demo_profiles:
            cursor.execute('INSERT INTO local_guides (user_id, city, country, university, employment, bio, hourly_rate, is_verified, verification_status, home_country, field_of_study, languages, cultural_background) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', profile_data)
        ...
        cursor.execute('''
            ...
            (student_id, home_country, field_of_study, languages, budget_range, preferred_session_types, cultural_adaptation_needs)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (student_id, 'India', 'Computer Science', 'English, Hindi', 'low', 'housing,transport', 'housing,transport,banking'))
